 ...
 */
export default function CertificateQuestion({
  question,
  answerCallback,
}: {
  question: Question;
  ...
  return (
    <Popup
      isOpen
      title={_("Secure connection error")}
      ...
      <Stack hasGutter>
        <StackItem>
          <Content component="p" isEditorial>
            {_(
              "There was a problem importing the following SSL certificate. Do you want to trust it?",
            )}
          </Content>
        </StackItem>
        <StackItem>
          <DescriptionList isHorizontal isCompact className={spacingStyles.mxLg}>
            ...
            <QuestionData label={_("Error")} value={question.data.error} />
            <QuestionData label={_("Issuer")} value={question.data.issuer_name} />
            <QuestionData label={_("Subject")} value={question.data.subject_name} />
